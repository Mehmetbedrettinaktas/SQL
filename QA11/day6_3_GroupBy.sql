...
select urun_adi, min(urun_miktari)as minimum_urun_mik, max(urun_miktari) as max_urun_mik
from manav 
where urun_adi is not null  -- null tabloda gormek istemesek
group by urun_adi  -- once grupladik
order by max_urun_mik;   -- sonra siraladik
...
select ulke,round(avg(maas) )as ortalama_maas from personel1 
group by ulke;

-- Maas ortalamasını bayanlar ve baylar olarak sorgulayınız
select cinsiyet,round(avg(maas)) as ortalama_mass from personel1
group by cinsiyet;
-- gruplamada ne isteniyorsa hem select hem de group by da yaziyoruz
...
select ulke,sehir, count(sehir)as calisan_sayisi from personel1
group by ulke, sehir;

-- Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını sorgulayınız.
select ulke, cinsiyet, count(*)  as calisan_sayisi,-- count (*) dedigimizde ulke ve cinsiyet stunlarnda her ne varsa sayar
round(avg(yas)) as yas_ortalamasi from personel1
...
select ulke, cinsiyet, count(*), round(avg(yas)) from personel1
where maas> 30000
group by ulke, cinsiyet; -- group by dan sonra where ( sart yazilmaz) bu yuzden group by dan once where kullanilir
...
where maas >30000
group by ulke, cinsiyet  -- ulke ve cinsiyete gore grupladik
order by ortalama_yas desc; -- ortalama yaşı büyükten küçüğe doğru sraladik.
